...
class MCPLoader(loader.ToolLoader):
    ...
    async def load_mcp_servers_from_db(self):
        ...
        self.ap.logger.info('Querying MCP servers from db...')
        result = await self.ap.persistence_mgr.execute_async(sqlalchemy.select(persistence_mcp.MCPServer))
        self.ap.logger.info('MCP servers query returned.')
        servers = result.all()
        self.ap.logger.info('Loaded %s MCP server configs from db.', len(servers))
