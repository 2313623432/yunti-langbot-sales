...
class SendResponseBackStage(stage.PipelineStage):
    ...
    def _special_case_model_uuid(self, query: pipeline_query.Query, workflow: dict[str, Any] | None = None) -> str:
        uuid = str(getattr(query, 'use_llm_model_uuid', '') or '').strip()
        if uuid:
            return uuid
        workflow = workflow or self._active_workflow(query) or {}
        if isinstance(workflow, dict):
            model_uuid = str(workflow.get('model_uuid') or '').strip()
            if model_uuid:
                return model_uuid
            metadata = workflow.get('metadata') if isinstance(workflow.get('metadata'), dict) else {}
            model_uuid = str(metadata.get('model_uuid') or '').strip()
            if model_uuid:
                return model_uuid
        return ''
    ...
    async def _special_case_message_chain(
        self,
    ...
    async def _apply_special_case_response(self, query: pipeline_query.Query) -> bool:
        ...
        if not case:
            return False
        query.resp_message_chain = [await self._special_case_message_chain(query, case)]
    ...
    def _handoff_reason(self, intent_data: dict[str, Any]) -> str:
        return str(
            intent_data.get('handoff_reason')
            or intent_data.get('reason')
            or intent_data.get('label')
            or 'handoff'
        )
    ...
    async def _ensure_sales_handoff_open(self, query: pipeline_query.Query, intent_data: dict[str, Any]) -> bool:
    ...
    async def _apply_handoff_response(self, query: pipeline_query.Query) -> bool:
        ...
        opened = await self._ensure_sales_handoff_open(query, intent_data)
